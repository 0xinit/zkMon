...
const TakeLoan = ({ signer, provider, account, tokenId }) => {
  ...
  useEffect(() => {}, [signer]); // Empty dependency array means this effect runs once when the component mounts
  ...
  const checkCanWithDrawTokens = async () => {
    ...
    const hexTokenAmount = await activeLendingContract?.borrowingPowerInUSD(
      address
    );
  ...
  const handleDeposit = async () => {
    await activeLendingContract.depositNFT(tokenContractAddress, tokenID, {
      gasLimit: 900000,
    });
  };
    ...
    <div className="bg-black text-green-400 min-h-screen p-4">
      ...
      <div className="max-w-md mx-auto mt-6">
        <form className="bg-green-900 shadow-md rounded px-8 pt-6 pb-8 mb-4">
          <div className="mb-4">
            <label
              className="block text-sm font-bold text-green-400 mb-2"
        ...
        <button
          className="font-semibold bg-green-500 w-full rounded-lg py-3 mt-5"
          onClick={checkCanWithDrawTokens}
        >
